...
class StreamingCoresetTest extends FreeSpec with Matchers {
  ...
  "The update" - {
    "should accept the first k'+1 points" in {
      ...
      val sState = new StreamingCoreset[Point](kPrime, 0, Distance.euclidean)

      sState.initializing should be (true)

      points.foreach { p =>
        sState.update(p) should be (true)
      }

      sState.initializing should be (false)
    }
    ...
    "after initialization, the threshold must be the minimum distance" in {
      ...
      sState.threshold should be (StreamingCoreset.minDistance[Point](
        sState._kernel, Distance.euclidean))
    }
  }

  "The delegate adding" - {
    "Should add the delegate only if thehre is space" in {
      val sState = new StreamingCoreset[Point](2, 1, Distance.euclidean)
  ...
  "The delegate merging" - {
    "Should add only the delegates there's space for" in {
      val sState = new StreamingCoreset[Point](2, 3, Distance.euclidean)
      sState._kernel(0) = Point(0)
      sState.addDelegate(0, Point(1)) should be (true)
      sState.addDelegate(0, Point(2)) should be (true)
      sState._kernel(1) = Point(3)
